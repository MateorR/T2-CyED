...
import ui.control.Blocked;

import java.util.ArrayList;
import java.util.List;

import static ui.BombermanGame.*;
import static ui.control.Menu.bombNumber;
...
public class Bomb extends Entity {
  ...
  public static void createEdge() {
    if (Blocked.blockDownBomb(bomb, 0)) {
      ...
      if (powerBomb > 0)
        for (int i = 1; i <= powerBomb; i++)
          if (Blocked.blockDownBomb(bomb, i)) {
    ...
    if (Blocked.blockLeftBomb(bomb, 0)) {
      ...
      if (powerBomb > 0)
        for (int i = 1; i <= powerBomb; i++)
          if (Blocked.blockLeftBomb(bomb, i)) {
